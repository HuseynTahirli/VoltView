...
type Alert = { id: number; type: string; message: string; created_at: string; resolved: boolean; };
    ...
    <div className="py-4 sm:py-6">
      ...
      <div className="rounded-xl border overflow-hidden" style={{ background: '#0a0a0a', borderColor: '#1a1a1a' }}>
        {loading ? (
          ...
          <div className="text-center text-[#555] py-10 text-sm">No active alerts</div>
        ) : (
          <>
            {/* Mobile card layout */}
            <div className="sm:hidden divide-y" style={{ borderColor: '#1a1a1a' }}>
              {alerts.map((a) => (
                <div key={a.id} className="p-4 space-y-2">
                  ...
                  <p className="text-xs text-[#555]">{new Date(a.created_at).toLocaleString()}</p>
                </div>
              ))}
            </div>
                ...
                <tbody>
                  {alerts.map((a) => (
                    <tr key={a.id} className="border-b hover:bg-[#0d0d0d] transition-colors" style={{ borderColor: '#111' }}>
                      ...
                      <td className="px-4 py-3 text-[#888]">{new Date(a.created_at).toLocaleString()}</td>
                      <td className="px-4 py-3">
                        {!a.resolved && (
                          <button onClick={() => handleResolve(a.id)}
